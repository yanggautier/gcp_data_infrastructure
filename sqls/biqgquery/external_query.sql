...
CREATE OR REPLACE TABLE `external_star_model.DIM_CUSTOMER` (
...
CREATE  OR REPLACE TABLE`external_star_model.DIM_SELLER` (
...
CREATE  OR REPLACE TABLE `external_star_model.DIM_PRODUCT` (
...
CREATE  OR REPLACE TABLE `external_star_model.DIM_DATE` (
...
CREATE  OR REPLACE TABLE `external_star_model.DIM_ORDER_STATUS` (
    status_key INT64 NOT NULL,
    order_status STRING NOT NULL
);

-- Dimension Méthode de Paiement (DIM_PAYMENT_TYPE)
CREATE  OR REPLACE TABLE `external_star_model.DIM_PAYMENT_TYPE` (
    payment_type_key INT64 NOT NULL,
    payment_type STRING NOT NULL
);

-- Création de la table de faits principale (FACT_ORDERS)
CREATE  OR REPLACE TABLE `external_star_model.FACT_ORDERS` (
...
INSERT INTO `external_star_model.DIM_CUSTOMER` (customer_key, customer_id, customer_unique_id, customer_zip_code_prefix, customer_city, customer_state)
SELECT 
...
FROM (
    SELECT DISTINCT 
        ...
        customer_state
    FROM  EXTERNAL_QUERY(
  'projects/vibrant-epsilon-454713-m2/locations/europe-west9/connections/mysql-federation-bigquery-connection',
  'SELECT * FROM customer')
);

-- Peuplement de DIM_SELLER
INSERT INTO `external_star_model.DIM_SELLER` (seller_key, seller_id, seller_zip_code_prefix)
SELECT 
...
INSERT INTO `external_star_model.DIM_PRODUCT` (
    product_key,
    product_id, 
...
LEFT JOIN 
    EXTERNAL_QUERY(
    'projects/vibrant-epsilon-454713-m2/locations/europe-west9/connections/mysql-federation-bigquery-connection',
    'SELECT * FROM category_name') t ON p.product_category_name = t.product_category_name;

-- Peuplement de DIM_ORDER_STATUS
INSERT INTO `external_star_model.DIM_ORDER_STATUS` (status_key, order_status)
SELECT 
...
INSERT INTO `external_star_model.DIM_PAYMENT_TYPE` (payment_type_key, payment_type)
SELECT 
...
INSERT INTO `external_star_model.DIM_DATE` (
    date_key,
    full_date,
    day_of_week,
    day_of_month,
...
INSERT INTO `external_star_model.FACT_ORDERS` (
    order_fact_key,
    order_id,
    customer_key,
    seller_key,
...
JOIN EXTERNAL_QUERY(
    'projects/vibrant-epsilon-454713-m2/locations/europe-west9/connections/mysql-federation-bigquery-connection',
    'SELECT * FROM online_order') o ON oi.order_id = o.order_id
JOIN EXTERNAL_QUERY(
    'projects/vibrant-epsilon-454713-m2/locations/europe-west9/connections/mysql-federation-bigquery-connection',
    'SELECT * FROM order_payment') op ON o.order_id = op.order_id
JOIN 
    `external_star_model.DIM_CUSTOMER` dc ON o.customer_id = dc.customer_id
JOIN 
    `external_star_model.DIM_SELLER` ds ON oi.seller_id = ds.seller_id
JOIN 
    `external_star_model.DIM_PRODUCT` dp ON oi.product_id = dp.product_id
JOIN 
    `external_star_model.DIM_ORDER_STATUS` dos ON o.order_status = dos.order_status
JOIN 
    `external_star_model.DIM_PAYMENT_TYPE` dpt ON op.payment_type = dpt.payment_type
WHERE o.order_purchase_timestamp IS NOT NULL;

-- Création de vues matérialisées pour optimiser les requêtes fréquentes
CREATE OR REPLACE MATERIALIZED VIEW `external_star_model.monthly_sales_by_category` AS
SELECT
    d.year,
    d.month,
    d.month_name,
...
FROM 
    `external_star_model.FACT_ORDERS` f
JOIN 
    `external_star_model.DIM_DATE` d ON f.purchase_date_key = d.date_key
JOIN 
    `external_star_model.DIM_PRODUCT` p ON f.product_key = p.product_key
GROUP BY 
